...
The algorithm works as long as there are fruits on the map. It checks what fruit is the closest to the player and computes the smallest path to it by Dijkstra's algorithm (you can read more about the algorith here: https://en.wikipedia.org/wiki/Dijkstra%27s_algorithm). Then, removes the eaten fruit, move the player's coordinate to the coordinate of the eaten fruit, and repeat the algorithm.
